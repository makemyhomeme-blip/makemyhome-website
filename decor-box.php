...
<head><meta charset="utf-8">
  ...
  <script type="application/ld+json">
  {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": "Decor Box", "item": "https://makemyhome.me/decor-box.html" }
    ]
  }
  </script>
  <script type="application/ld+json">
  {
    ...
    "provider": {
      ...
      "name": "Make My Home Decor",
      "url": "https://makemyhome.me/",
      "telephone": "+38269105222",
  ...
  <style>
    ...
    .db-factory-img{border-radius:20px;overflow:hidden;background:var(--dark-2);}
...
require_once __DIR__ . '/php/dimenzije.php';
    /* Velicinu ovih slika vlasnik podesava u adminu (Decor Box → Velicina slika).
       ...
       izbor sacuvan u data/decor-box-style.json, a ovdje se samo ispisuje.
       Sync sada nema sta da obrise. */
    $dbCfg = json_decode(@file_get_contents(__DIR__ . '/data/decor-box-style.json'), true) ?: [];
    foreach ([['banner', '.db-intro-img'], ['fabrika', '.db-factory-img']] as [$slot, $sel]) {
        $c    = $dbCfg[$slot] ?? [];
        $fit  = ($c['fit'] ?? 'cover') === 'contain' ? 'contain' : 'cover';
        $fiks = ($c['mode'] ?? 'auto') === 'fixed';
        $h    = max(160, min(900, (int)($c['height'] ?? 340)));
        if ($fiks) {
            echo "    {$sel}{min-height:{$h}px;}\n";
            echo "    {$sel} img{height:{$h}px;object-fit:{$fit};}\n";
        } else {
            echo "    {$sel}{min-height:auto;}\n";
            echo "    {$sel} img{height:auto;object-fit:{$fit};}\n";
        }
    }
    ?>
    ...
    /* Kad u istom polju stoje dva ili tri linka, bez razmaka se slijevaju u
       ...
       Razmak ide na SAM LINK, ne na celiju: display:flex na <td> ukida
...
<section class="db-factory">
  <div class="container">
    <div class="db-factory-grid">
      <div class="animate-on-scroll">
        <span class="db-eyebrow">Naša fabrika u Beogradu</span>
        <div class="gold-line"></div>
        <h2 class="section-title" style="color:#fff;">Sopstvena proizvodnja</h2>
        <p>
          Sopstvena proizvodnja u Beogradu garancija je kvaliteta, pouzdanih rokova i fleksibilnosti za svaki projekat.
          Kontrolišemo cijeli proces – od materijala do isporuke – kako bismo vam obezbijedili dosljedan kvalitet i sigurnost u realizaciji.
        </p>
        <div class="db-badges">
          <div class="db-badge"><i class="fas fa-industry"></i><span>Sopstvena proizvodnja</span></div>
          <div class="db-badge"><i class="fas fa-award"></i><span>Visoki standardi kvaliteta</span></div>
          <div class="db-badge"><i class="fas fa-truck-fast"></i><span>Brza isporuka i pouzdanost</span></div>
        </div>
      </div>
