...
- ukupno pogodaka: **26**
- **bez zastite** (moze pregaziti ono sto je server ispisao): **18**
- sa zastitom: 8
...
| `products.php` | 834 | `(vrh fajla)` | textContent = | `var ct=document.getElementById('cat-title');if(ct)ct.textContent=n;` |
| `products.php` | 839 | `(vrh fajla)` | display='none' | `var cg=document.getElementById('category-grid');if(cg)cg.style.display='none';` |
...
| `js/products.js` | 500 | `id` | textContent = | `if (breadcrumb) breadcrumb.textContent = product.name;` |
| `js/products.js` | 604 | `dotWrap` | display='none' | `onerror="this.onerror=null;this.closest(&quot;.gallery-thumb&quot;).style.display='none'">` |

## Sa zastitom — bezopasno

| fajl | linija | funkcija | obrazac |
|---|---|---|---|
| `products.php` | 670 | `(vrh fajla)` | textContent = |
| `js/products.js` | 336 | `showCategoryGrid` | display='none' |
| `js/products.js` | 384 | `showCategoryProducts` | display='none' |
| `js/products.js` | 1325 | `styleMatchHtml` | innerHTML = |
| `js/products.js` | 1522 | `totalCost` | innerHTML = |
| `js/products.js` | 308 | `showSubcategoryGrid` | filter/slice nad listom |
| `js/products.js` | 416 | `showCategoryProducts` | filter/slice nad listom |
| `js/products.js` | 1520 | `totalCost` | filter/slice nad listom |
