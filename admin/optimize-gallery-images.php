<?php
/**
 * Make My Home – Optimizacija galerijskih slika (smanji veličinu fajla)
 * Diraju se SAMO slike iz "gallery" polja proizvoda (slike u prostoru/detalji).
 * Glavna slika svakog proizvoda (polje "image") se NIKAD ne dira.
...
// Zaštićene slike (glavna slika svakog proizvoda) - NIKAD se ne diraju
$protected = [];
foreach ($products as $p) {
    if (!empty($p['image'])) $protected[basename($p['image'])] = true;
}

// Slike za obradu (samo iz "gallery" polja, izuzimajući one koje su i negdje glavne)
$toProcess = [];
foreach ($products as $p) {
    foreach (($p['gallery'] ?? []) as $g) {
        if (!$g) continue;
        if (isset($protected[basename($g)])) continue;
        $toProcess[$g] = true;
    }
}
...
echo "=== Optimizacija galerijskih slika ===\n";
echo "Zaštićenih glavnih slika: " . count($protected) . "\n";
echo "Kandidata za obradu (gallery): " . count($toProcess) . "\n";
echo $dryRun
...
foreach (array_keys($toProcess) as $relPath) {
    ...
    printf("%-55s %6d KB -> %6d KB  (-%d%%)\n", $relPath, $sizeBefore / 1024, $sizeAfter / 1024, $pct);
}
